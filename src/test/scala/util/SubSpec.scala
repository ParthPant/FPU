...
class FastSubtractorPipelinedSpec extends AnyFlatSpec with ChiselScalatestTester {
    ...
    for (w <- List(8, 16, 32, 64)) { 
        it should s"Subtract $w-bit numbers" in {
            for (i <- 1 to 5) {
                ...
                test (new FastSubtractorPipelined(w, 4)).withAnnotations(Seq(WriteVcdAnnotation)) { c =>
                    val a = BigInt(w, r)
                    val b = BigInt(w, r)
                    val cin = r.nextInt(2) 
                    val d = a-b-cin
